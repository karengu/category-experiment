...
write_csv(rs.wp.ais, path = "webppl/output/ais_allConds_nComponents_50k.csv")
```
...
rs.wp.ais %>% 
  group_by(n_c, iter) %>%
  summarize(marg_llh = sum(marg_ll)) %>% kable()
```
...
rs.wp.condition.predictives <- rs.wp.condition %>%
  ...
  rowwise() %>%
  mutate(component1 = rbernoulli(n = 1, p = phi),
         sample1 = rbeta(n = 1, shape1 = a1, shape2 = b1),
         sample2 = rbeta(n = 1, shape1 = a2, shape2 = b2),
