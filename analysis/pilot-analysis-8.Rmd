...
df.attention %>% filter(pass) %>% 
  group_by(attemptNumber) %>% count()

first.attempt.pass <- df.attention %>% filter(pass, attemptNumber == 1) %>% pull(workerid)

pass.workers <- df.attention %>% filter(pass) %>% pull(workerid)
# left_join(
#   data.frame(workerid = unique(df.attention$workerid)),
#   df.attention %>% filter(pass)
# )
# 
# 
...
```{r}
fig.obj <- df.trials %>%
  filter(workerid %in% first.attempt.pass) %>%
